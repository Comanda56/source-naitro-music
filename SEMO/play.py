...
async def aii(client: Client, message):
   ...
   try:
    ...
    raw_list = []
    async for msg in user.get_chat_history("nAiTrOMUSIC"):
        if msg.audio:
          raw_list.append(msg)
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "DARK_x7"
    videoid = None
    link = None
     ...
     if await is_served_call(client, message.chat.id):
         ...
         if message.from_user:
          if message.from_user.photo:
           ...
           ahmed = await client.get_chat("DARK_x7")
