...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "DARK_x7":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "نايترو")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطو نايترو للشات  💎 .\n♪ مرحبا بك : @DARK_x7  💎 .**")
    dev = await get_dev(bot_username)
...
async def DARK_x7(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
"مره واحد اسمو نايترو  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 ​ꪀꪖ𝓲𝓽𝘳​ꪮ ](t.me/NA_44_ROo) 💎 .
么  [𝒅𝒆𝒗 ​ꪀꪖ𝓲𝓽𝘳​ꪮ ](t.me/DARK_x7) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور نايترو","نايترو","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="DARK_x7")
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["DARK_x7"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["DARK_x7"]:
   ...
   if message.from_user.username == "DARK_x7":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور نايترو  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @DARK_x7yy  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @DARK_x7yy  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @DARK_x7yy  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["DARK_x7"]:
         await message.reply_text("**♪ رتبتك هي : المطور نايترو  💎 .**")
         return
